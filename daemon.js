...
class Report {
  ...
  // FIXME: number of server is not correct. much to high. Threads?
  activitySummary() {
    let str = '\n\r This cycle:'
  ...
  serverStatus(servers,failed,skipped) {
    ...
    let cantHackButPrepping = servers.filter(s => !s.canHack() && s.isPrepping());
    let prepping = servers.filter(s => s.isPrepping());
    ...
    if (skipped.length > 0)
        str += `, ${skipped.length} skipped for now (time, RAM, or target + prepping cap reached)`;
    if (failed.length > 0)
        str += `, ${failed.length} failed to be scheduled (insufficient RAM?).`;
    if (cantHack.length > 0)
        str += `\n > ${cantHack.length} cannot be hacked (${cantHackButPrepping.length} prepping, ` + `${cantHackButPrepped.length} prepped, next unlock at Hack ${this.lowestUnhackable})`;
...
export async function main(ns) {
    ...
    asynchronousHelpers = [
        ...
        { name: "stockmaster.js", shouldRun: () => ns.getServerMaxRam("home") > 32, args: openTailWindows ? ["--show-market-summary"] : [], tail: openTailWindows }, // Start our stockmaster
        { name: "hacknet-upgrade-manager.js", shouldRun: () => ns.getServerMaxRam("home") > 32, args: ["-c", "--max-payoff-time", "1h"] }, // Kickstart hash income by buying everything with up to 1h payoff time immediately
...
async function refreshDynamicServerData(ns, serverNames) {
    ...
    if (ns.getServerMaxRam("home") >= 16) {
        ...
        log(ns,"Insufficient RAM for parallel execution of analyze-hack! Start analyze-hack.js manually until upgrading HOME to > 8GB",true,"error")
    }
...
export function getNumPortCrackers() {
